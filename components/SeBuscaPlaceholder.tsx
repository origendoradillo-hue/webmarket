  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center overflow-hidden bg-hueso">
      ...
      <div className={`relative flex flex-col items-center ${compact ? "gap-1" : "gap-2 px-4 text-center"}`}>
        <div className={`flex items-center justify-center rounded-full bg-golfo/15 ${compact ? "h-9 w-9" : "h-16 w-16"}`}>
          <i className={`ti ti-search text-golfo ${compact ? "text-lg" : "text-3xl"}`} aria-hidden />
        </div>
        {!compact && (
          <>
            <h3 className="font-slab text-xl font-semibold text-oliva-dd">Se busca</h3>
