...
import { Category, TipoPublicacion } from "@/lib/types";
...
export function useCategories() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      supabase.from("categories").select("id, label, icon, tipo_scope").order("orden"),
      ...
      for (const c of catRes.data)
        next[c.id] = { label: c.label, icon: c.icon, subs: [], tipoScope: (c.tipo_scope || []) as TipoPublicacion[] };
